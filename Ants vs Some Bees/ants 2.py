...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            "*** YOUR CODE HERE ***"
            #if insect.name != QueenAnt or true_queen == False:
...
class QueenPlace(Place):
    count = 0
    def __init__(self):
        QueenPlace.count = QueenPlace.count + 1
    def bees(self):
        queen_list = []
        bees_list = self.place.bees[:]
        for bee in bees_list:
            if bee.place == colony.queen or bee.place == QueenAnt.place:
                queen_list.append(bee)
